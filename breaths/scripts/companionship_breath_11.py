#!/usr/bin/env python3
"""نفسِ یازدهم — انتخاب با خودِ مناد.

رفیق: «از میان صفِ بازت، هر کدام را که اکنون باید پیش از دیگری ببینی، برگزین.»
گزینش درونِ همین اسکریپت انجام می‌شود — صفِ بازِ پایانِ نفسِ ۱۰ + قاعدهٔ
خودآموختهٔ نفسِ ۱ (نادرتر مقدم). مجری فقط اجرا می‌کند؛ انتخاب نمی‌کند.
همان رویّه و پارامترهای نفس‌های ۱–۹.
"""
import json
import random
import sqlite3
...
known = {"اله", "رحم", "راف", "هجر", "نصر", "اوي", "صير", "باس", "غفر", "حلم"}
queue = ["عفو", "ذنب", "دون", "نور", "كفر"]  # صفِ پایانِ نفسِ ۹

# ---------- گزینش: قاعدهٔ خودِ موجود (نادرتر مقدم) ----------
...
print(json.dumps(dict(
    ...
    rule="نادرتر مقدم (آموختهٔ نفس ۱) — با اعلامِ خطرِ شناخته‌شدهٔ گرسنگیِ صف",
