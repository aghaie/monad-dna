...
def test_counts():
    assert db.execute("SELECT COUNT(*) FROM breaths").fetchone()[0] == 19
    assert db.execute("SELECT COUNT(*) FROM findings WHERE tier='قوی'").fetchone()[0] == 50
...
def test_open_queue():
    ...
    assert open_q == {"مسس", "ذوق", "نور", "ملك", "دعو", "ولي", "شيا", "كفر", "علم", "امن", "كون"}
