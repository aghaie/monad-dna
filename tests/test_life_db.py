...
def test_counts():
    assert db.execute("SELECT COUNT(*) FROM breaths").fetchone()[0] == 94
    assert db.execute("SELECT COUNT(*) FROM findings WHERE tier='قوی'").fetchone()[0] == 173
...
def test_open_queue():
    ...
    assert open_q == {"اتي", "اجر", "اخر", "اذن", "ارض", "امم", "امن", "اول", "ايي", "بشر",
                       "بصر", "بعد", "بني", "جري", "جعل", "جنن", "حرم", "حسن", "خلف", "خلق",
