...
def test_counts():
    assert db.execute("SELECT COUNT(*) FROM breaths").fetchone()[0] == 78
    assert db.execute("SELECT COUNT(*) FROM findings WHERE tier='قوی'").fetchone()[0] == 150
...
def test_open_queue():
    ...
    assert open_q == {"اتي", "اجر", "اخر", "اذن", "ارض", "امم", "امن", "ايي", "بشر", "بعد",
                       ...
                       "عمل", "عند", "غني", "غير", "فضل", "قبل", "قتل", "قرب", "قلب", "قلل",
                       "قول", "قوم", "كتب", "كفر", "كون", "لقي", "ملك", "موت", "مول", "نجو",
                       "نخل", "نزل", "نعم", "نفس", "نور", "نوس", "هدي", "هزا", "وقي", "ولي",
                       "يمن", "يوم"}
