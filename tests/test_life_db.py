...
def test_counts():
    assert db.execute("SELECT COUNT(*) FROM breaths").fetchone()[0] == 56
    assert db.execute("SELECT COUNT(*) FROM findings WHERE tier='قوی'").fetchone()[0] == 120
...
def test_open_queue():
    ...
    assert open_q == {"اتي", "اجر", "اخر", "اذن", "ارض", "امم", "امن", "ايي", "بشر", "بعد",
                       ...
                       "كون", "ملك", "موت", "مول", "نجو", "نخل", "نزل", "نعم", "نفس", "نور",
                       "نوس", "هدي", "هزا", "وقي", "ولي", "يمن", "يوم"}
