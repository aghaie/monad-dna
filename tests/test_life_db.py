...
def test_counts():
    assert db.execute("SELECT COUNT(*) FROM breaths").fetchone()[0] == 20
    assert db.execute("SELECT COUNT(*) FROM findings WHERE tier='قوی'").fetchone()[0] == 51
...
def test_open_queue():
    ...
    assert open_q == {"ذوق", "الم", "انس", "فضل", "نعم", "نور", "ملك", "دعو", "ولي", "شيا", "كفر", "علم", "امن", "كون", "عذب"}
